...
const loadServiceAccount = (filePath) => {
  if (fs.existsSync(filePath)) {
    try {
      ...
      console.error(
        `🔥 Failed to parse service account file ${filePath}:`,
        err.message
      );
    }
  } else {
...
try {
  const vendorServiceAccountPath = path.resolve(
    "./rootmit-vendor-firebase-adminsdk-fbsvc-cdd5c5ac9a.json"
  );
  const userServiceAccountPath = path.resolve(
    "./rootmituser-firebase-adminsdk-fbsvc-2b9c6ac052.json"
  );
  const deliveryServiceAccountPath = path.resolve(
    "./delivery-partner-59f51-firebase-adminsdk-fbsvc-2eb3bea432.json"
  );
...
const sendNotification = async ({
  ...
  try {
    ...
    if (records.length === 0) {
      return {
        success: false,
        message: `No ${type}s found with valid FCM tokens`,
      };
    }
    ...
    if (tokens.length === 0) {
      return {
        success: false,
        message: `No valid FCM tokens found for ${type}`,
      };
    }
    ...
    console.log(
      `✅ Sent ${type} notification → Success: ${response.successCount}, Failed: ${response.failureCount}`
    );
    ...
    if (response.failureCount > 0) {
      ...
      if (failedTokens.length > 0) {
        ...
        console.log(
          `🧹 Cleaned up ${failedTokens.length} invalid FCM tokens from ${table}`
        );
      }
    }
...
export const sendMultiTypeNotification = async ({
  targets,
  title,
  message,
  data = {},
  ...
  try {
    ...
    return {
      success: false,
      message: "Failed to send multi-type notification",
      error: error.message,
    };
  }
};
